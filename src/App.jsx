...
import { Search, Plus, Lock, Upload, Download, Trash2, X, Guitar, Play, FileText, ShieldCheck, LogOut, ChevronDown, ChevronRight, Cloud, AlertTriangle } from 'lucide-react'
...
export default function App(){
  ...
  async function addSong(e){
    ...
    setError('')
    try {
      let fileData = { tabUrl: '', tabName: '', tabType: '' }
      ...
      chooseLevel(saved.level)
      setSelectedId(saved.id)
      setForm(blankForm())
      setAdminOpen(false)
    } catch (err) { setError(err.message || 'Erreur pendant la publication.') }
  }
  ...
  return <div className="app">
    ...
    <main className="main">
      <header className="topbar">
        ...
        <div className="top-actions"><span className={`cloud-status ${supabase ? 'ok' : 'ko'}`}><Cloud size={16}/>{supabase ? 'Cloud' : 'Setup'}</span><button onClick={()=>setAdminOpen(true)} className="admin-btn"><Lock size={17}/> Admin</button></div>
      </header>
      ...
      <SongDetail song={selectedSong} isAdmin={!!session} onDelete={removeSong}/>
    </main>

    {adminOpen && <AdminModal session={session} onClose={()=>setAdminOpen(false)} form={form} setForm={setForm} addSong={addSong} refresh={fetchSongs}/>} 
  </div>
}
...
function renderTab(song){
  ...
  if(song.tabUrl && song.tabType === 'application/pdf') return <><iframe className="pdf" src={song.tabUrl}/><a className="download" href={song.tabUrl} target="_blank" rel="noreferrer"><Download size={16}/> Ouvrir le PDF</a></>
  return <div className="placeholder"><FileText/>Aucune tablature ajoutée.</div>
}
function AdminModal({session,onClose,form,setForm,addSong,refresh}){
  ...
  return <div className="modal-backdrop"><div className="modal"><button className="close" onClick={onClose}><X/></button><h2>Espace admin</h2>
    {!supabase ? <div className="notice error"><AlertTriangle size={18}/> Configure Supabase dans Vercel avant d’utiliser l’admin.</div> : !session ? <form onSubmit={login} className="admin-login"><ShieldCheck/><p>Connecte-toi avec l’utilisateur créé dans Supabase Auth.</p><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email admin"/><input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Mot de passe"/>{authError && <p className="auth-error">{authError}</p>}<button disabled={busy} className="gold-button">{busy ? 'Connexion...' : 'Entrer'}</button></form> : <>
      ...
      <form onSubmit={addSong} className="admin-form">
        ...
        <label className="upload"><Upload/> Uploader tablature PNG/JPG/PDF<input type="file" accept="image/png,image/jpeg,application/pdf" onChange={e=>setForm({...form, tabFile:e.target.files?.[0] || null, tabName:e.target.files?.[0]?.name || ''})}/></label>
